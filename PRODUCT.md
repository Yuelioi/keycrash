...
- 本地运行、无需账户或网络服务；Release GUI 以普通权限启动，只在普通定位未命中时按需请求管理员权限覆盖高完整性注册者。
...
- owner 命中后显示不带 `.exe` 的软件名与完整路径，并提供“打开文件位置”；右上角“？”在同一窗口解释三步流程与触发风险。
- Windows 公开 API 无法从 `RegisterHotKey` 失败结果直接取得占用进程 PID；归因必须基于目标进程实际取走匹配 `WM_HOTKEY` 的运行时证据。
- 深度定位会先后通过普通与按需管理员 x64/x86 Helper 使用 `SendInput`，最多触发四次完整组合，并在 `PM_REMOVE` 阶段尽量把匹配消息改为 `WM_NULL`；低级 Hook、Raw Input、驱动或忽略模拟输入的软件仍可能执行动作或无法归因。
