...
- 不把 GUI 改成 `requireAdministrator`。权限覆盖仍由一次“定位占用软件”操作自动完成；界面不再把普通/管理员两轮作为用户需要理解的步骤暴露出来。
...
- “打开文件位置”调用 Explorer 时使用其原始参数语法 `/select,"<path>"`；`Command::arg` 会把含空格路径的整个参数再次引用，导致 `C:\Program Files\...` 这类路径无法选中文件。
